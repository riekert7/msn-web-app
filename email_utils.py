...
def send_admin_new_submission_email(submission_data: dict) -> bool:
    """Send admin notification with approve/deny links and a secure link to proof-of-payment."""
    ...
    view_url = f"{base_url}/view-payment/{sid}?token={token}"
    ...
    text = (
        ...
        f"View proof of payment: {view_url}\n\n"
    ...
    html = (
        ...
        f'<p><a href="{view_url}" style="color:#667eea;">View proof of payment</a></p>'
        f"{reach_html}"
    ...
    return _send_email(admin_email, subject, text, html)
